...
export class Tubular {
  ...
  static async create(archivePath: string, youtubei: Youtubei) {
    ...
    await zip.extract(archivePath, tmpdirPath, {
      onEntry(event) {
        if (event.entryName !== "newpipe.db" && event.entryName !== "preferences.json") {
  ...
  private async importSubscriptions(subscriptions: Subscriptions) {
    ...
    const createSubscription = async (channelId: string) => {
      const channel = await pRetry(async () => await this.youtubei.getChannel(channelId), { retries: 3 })
      if (!channel) {
  ...
  private async importPlaylists(playlists: Playlists) {
    ...
    const createPlaylist = async (name: string, videoIds: string[]) => {
      ...
      const playlistRowId = this.insertPlaylist(name, videos[0].streamRowId)
  ...
  private async insertStream(videoId: string) {
    ...
    const video = await pRetry(async () => await this.youtubei.getVideo(videoId), { retries: 3 })
    if (!video) {
    ...
    const id = this.db
      .query(
        'INSERT INTO streams (service_id, url, title, stream_type, duration, uploader, uploader_url, thumbnail_url, view_count) VALUES (0, $url, $title, "VIDEO_STREAM", $duration, $uploader, $uploaderUrl, $thumbnailUrl, $viewCount)',
      )
      .run({
  ...
  private insertPlaylist(name: string, thumbnailStreamRowId: number) {
    const id = this.db
      .query(
        "INSERT INTO playlists (name, is_thumbnail_permanent, thumbnail_stream_id, display_index) VALUES (?, 0, ?, -1)",
      )
      .run(name, thumbnailStreamRowId).lastInsertRowid
    typia.assertGuard<number>(id)
    return id
  }
